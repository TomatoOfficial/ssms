...
output_version = 'outv126-'
out_file_time = datetime.datetime.now().strftime('%Y.%m.%d %H.%M.%S')
out_file = folder_dict + '\\' + 'output\\' + output_version + out_file_time + '.csv'
stuData = []
...
#        print("Verifying data...")
        ...
        print("\nCSV File '" + output_version + out_file_time + '.csv' + f"' has been verified. Total rows: {row_count+1}\n({out_file})\n\n")

# 抽选班长选择表格文件
def select_file(output_dict):
    global selected_file_path
    # 列出指定目录下的所有文件
    files = [f for f in os.listdir(output_dict) if os.path.isfile(os.path.join(output_dict, f))]
    ...
    while True:
        try:
            ...
            if 1 <= choice <= len(files):
                ...
                selected_file_path = os.path.join(output_dict, selected_file)
